...
#' @param feature_colname String the name of the feature column that the celltypes are under
...
#' @export
select_celltypes <- function(sce_object, celltypes, feature_colname = "Phenotype", keep = TRUE){
  ...
  if (keep) slim_sce <- sce[, (sce[[feature_colname]] %in% celltypes)]
  else slim_sce <- sce[, !(sce[[feature_colname]] %in% celltypes)]
