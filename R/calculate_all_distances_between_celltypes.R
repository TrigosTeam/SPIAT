#' calculate_all_distances_between_celltypes
#'
#' @description Returns the distances between cells of different cell types
#'
#' @param sce_object SingleCellExperiment object in the form of the output of format_image_to_sce
#' @param remove_other If TRUE, the plotting will exclude undefined cells
...
#' @examples
#' distances <- calculate_all_distances_between_celltypes(SPIAT::formatted_image, remove_other = TRUE, cell_types_of_interest = c("CD3,CD4", "CD3,CD8"))
#' @export

calculate_all_distances_between_celltypes <- function(sce_object, remove_other = TRUE, cell_types_of_interest = NULL){
  ...
  if(!is.null(cell_types_of_interest)){
    ...
    dat <- dat[dat$Cell.Type %in% unique_cell_types_selected,]
  }
  #CHECK
  if (nrow(dat) == 0) {
  ...
  dat <- dat[,c("Cell.ID","Cell.Type", "Cell.X.Position", "Cell.Y.Position")]
  dat <- dat[dat$Cell.Type != "",]
  dat$Cell.Type <- as.character(dat$Cell.Type)
  if (isTRUE(remove_other)){
    dat <- dat[dat$Cell.Type != "OTHER",]
  }
  
  #Creates a list of the number of cell types with all their corresponding cell ID's
  cell_types = list()
  for (eachType in unique(dat$Cell.Type)) {
    cell_types[[eachType]] = as.character(dat$Cell.ID[dat$Cell.Type == eachType])
  }
