#' entropy
#'
#' @description Produces a dataframe of entropies for all reference cells or an entropy number for the whole image. 
...
entropy <- function(sce_object, types_of_interest, column, radius = NULL){
  if (!is.null(radius)){
    ...
    n_cells <- SPIAT::number_of_cells_within_radius(sce_object, reference_marker = reference_marker,
