...
mixing_score_summary <- function(sce_object, reference_marker, target_marker, radius=20, column)
{
  ...
  for (i in reference_marker) {
    reference_cells <- formatte_data[grepl(i, formatted_data[ ,column]), ]
    for (j in target_marker) {
      if (i == j) {next}
      tryCatch({
        target_cells <- formatted_data[grepl(j, formatted_data[ ,column]), ]
        target_cells <- target_cells[!grepl(i, target_cells[ ,column]), ]
        reference_cells <- reference_cells[!grepl(j, reference_cells[ ,column]), ]
