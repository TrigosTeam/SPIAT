...
#'   Options: "INFORM", "HALO", "Visium", "CODEX" "cellprolifer, or "general"
#' @param path String of the path location of either HALO csv file, INFORM text
#'   file or CODEX csv file. If 10X Visium transcriptomics data, path to the
#'   "filtered_feature_bc_matrix" folder generated by CellRanger.
#' @param path_to_visium_coordinates For 10X Visium data. String of the path to
#'   the "tissue_positions_list.csv" generated by CellRanger.
#' @param path_to_codex_cell_phenotypes For CODEX data. String of the path to
#'   the Cluster ID/Cell type file.
#' @param markers For INFORM and HALO formats. Vector containing the markers
#'   used for staining. Must match the order of the 'markers' parameter
#' @param locations Vector containing the locations of markers used for
...
#' @param dye_columns_interest For HALO formats. Use if locations is not
...
#' @param intensity_columns_interest For HALO formats. Use if locations is not
...
#' @param intensity_matrix For the "general" format. A matrix of marker
#'   intensities or gene expression where the column names are the Cell IDs, and
#'   the rownames the marker or genes.
#' @param phenotypes A vector of cell phenotypes in the same order in which they
#'   appear in intensity_matrix. If no phenotypes available, then a vector of NA
#'   can be used as input. Note that the combination of markers (e.g. CD3,CD4)
#'   needs to be used instead of the cell type name (e.g. helper T cells)
#' @param coord_x A vector with the X coordinates of the cells. The cells must
...
format_image_to_sce <- function(format = "INFORM", 
  ...
  if (format == "HALO"){
    ...
    raw_image <- Seurat::Read10X(data.dir = path)
    ...
    #read in the image
    image <- read.delim::read.csv(path)
