...
plot_cell_percentages <- function(cell_proportions, cells_to_exclude =NULL, 
  ...
  cell_percentages_full_plot <-
    ggplot(cell_proportions,aes(x = stats::reorder({{cellprop_colname}}, Percentage), 
  ...
  if(!is.null(cells_to_exclude)){
    cell_proportions_no_tumour <- cell_proportions[!cell_proportions$Cell_type %in% cells_to_exclude,]
    cell_proportions_no_tumour$Proportion <- NULL
    cell_proportions_no_tumour$Percentage <- (cell_proportions_no_tumour$Number_of_celltype/sum(cell_proportions_no_tumour$Number_of_celltype))*100
    ...
    cell_percentages_no_tumour_plot <-
      ggplot(cell_proportions_no_tumour, aes(x = stats::reorder({{cellprop_colname}}, Percentage), 
                                             y = Percentage, fill=Cell_type)) +
      geom_bar(stat = 'identity') +
