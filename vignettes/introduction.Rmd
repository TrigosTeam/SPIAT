...
* PDL-1 = immune checkpoint cells (need to change this)
* OTHER = other cell types

AMACR identifies which cells in the image are prostate cancer cells. CD3,CD4, CD3,CD8, CD3 which types of T cells (immune cells) are present, and PDL-1 is used to help determine if the cancer may benefit from immunotherapy treatment. (need to change)
...
predicted_image <- predict_phenotypes(sce_object = simulated_image,
                                      thresholds = NULL,
                                      tumour_marker = "AMACR",
                                      baseline_markers = c("CD3", "CD4", "CD8", "FOXP3"),
...
Similar plots are generated for other markers. For example, the following plots are for AMACR.
...
data_subset <- select_celltypes(formatted_image, keep=TRUE,
                                celltypes = c("AMACR","CD3,CD8", "CD3,CD4",
                                             "CD3,CD4,FOXP3"),
...
```
ggplot works (not sure what's wrong with the original function...)
```{r}
ggplot(p_cells, aes(x=Cell_type, y=Percentage))+
...
plot_cell_categories(formatted_image, c("Tumour", "T_helper", "T_cyto", "T_reg"), 
                     my_colors, "Cell.Type")
```
...
distances <- calculate_all_distances_between_cell_types(formatted_image, 
                                                        cell_types_of_interest = c("Tumour", "T_reg"),
                                                        feature_colname = "Cell.Type")
```
...
average_percentage_of_cells_within_radius(formatted_image, 
                                          reference_celltype = "T_helper", 
                                          target_celltype = "T_cyto", 
...
```{r}
mixing_score_summary(formatted_image, reference_marker = "T_helper", 
                     target_marker = "T_cyto", radius=100, feature_colname ="Cell.Type")
```
...
```{r}
calculate_entropy(formatted_image, cell_types_of_interest = c("T_helper","T_cyto"), 
                  feature_colname = "Cell.Type")
```
...
gradient_entropy <- compute_gradient(formatted_image, radii = gradient_positions, 
                                     FUN = calculate_entropy, 
                                     cell_types_of_interest = c("T_helper","T_cyto"),
                                     feature_colname = "Cell.Type")
```
...
```{r}
names_of_immune_cells <- c("T_helper", "T_cyto","T_reg")
...
```{r}
plot_cell_categories(formatted_structure,
...
```{r}
clusters <- identify_cell_clusters(formatted_image, cell_types_of_interest = c("T_helper", "T_cyto"), radius = 100, feature_colname = "Cell.Type")
```
