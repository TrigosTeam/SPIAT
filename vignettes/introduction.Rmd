...
citation("SPIAT")
```
Can you add the citation to the Github package itself?
...
```
The formatted image now contains phenotypes location and marker intensitiy information of 3 cells. Note that if users want to define cell IDs, the cell IDs should be defined as the colnames of the intensity matrix. The order of the rows of the metadata should correspond to the order of the colnames of the intensity matrix. The function will automatically assing rownames to the metadata
...
```
Print out the first few lines so they can see what the output looks like

The result can be interpreted as the 'average distance between cells of type A and type B is XXXXX (add here the value)'.
...
```
Print out the first few lines so they can see what the output looks like. Probably also say something like:
...
The result can be interpreted as the 'average minimum distance between cells of type A and type B is XXXXX (add here the value)'.
...
There is another pattern in cross K curve which has not been previously appreciated, which is when there is a "ring" of one cell type, generally immune cells, surrounding the area of another cell type, generally tumour cells. For this pattern, the observed and expected curves in cross K function cross or intersect. 

Perhaps we add the plot of the cross K function crossing?
...
We should add a visualization for this section. I think we could add again the dot plot of the image we are working with, and say "X cells are forming a ring".
What does crossing_of_crossK do if there are multiple cell types? don't you need to define which are the populations of interest? Oh I see, I think you define when you create df_cross. I have added above
...
grid <- grid_metrics(defined_image, FUN = calculate_entropy, n_split = 20,
                     ...
                     feature_colname = "Cell.Type")
```
The title of the plot is incomplete. If it is due to the rendering of the tutorial, then don't worry, but it could be a problem in the function.
...
```{r, echo=FALSE}
knitr::include_graphics("tumour_structure.png")
```
can you update the image so we use the same terms? I think Internal and External margins are better.
...
We can test for the presence of neighbourhoods using ANNI. We can calculate the ANNI with the function `average_nearest_neighbor_index`, which will  output whether there is a clear neighbourhood (clustered) or unclear (dispersed), along with a P value for the estimate. 

We can define a neighbourhood as being of a single cell type (e.g. Immune1) or a combination of cell types. In cases of a combination, we need to create a novel cell type.... (can you update the example based on what I have written?)

```{r}
average_nearest_neighbor_index(formatted_image, reference_cell="Tumour", 
                               feature_colname="Cell.Type", p.val = 5e-6)
```
Can we run this without adding a p-value parameter? If no, then explain what that parameter does.
