...
df_cross <- bivariate_dependence(formatted_image, method = "Kcross",
                     phenotypes = c("CD4","CD8"), column ="Cell.Type",                      plot=FALSE)
```
...
```{r}
entropy(formatted_image, types_of_interest = c("CD4","CD8"), 
                                              column = "Cell.Type")
```
...
mixing_score_summary(formatted_image,
                                     ...
                                     target_marker = "CD8", radius=100, column="Cell.Type")
```
...
```{r}
# update with jojo's new functions
...
```{r}
clusters <- identify_cell_clusters(formatted_image, cell_types_of_interest = c("CD3", "CD4"), radius = 100, column = "Cell.Type")
```
