...
[Bioconductor](http://bioconductor.org) repository for packages. You can
install the lasted development version from Github.
...
no phenotypes are available. The user also need to provide separate vectors 
with theX and Y coordinates of the cells (`coord_x` and `coord_y`). The cells 
...
"CD3,CD4"), HALO uses a binary system of 1 if the cell is positive for
the marker and 0 otherwise.

`format_image_to_spe()` fpr "HALO" format collapses HALO encoded phenotypes into an
...
`colData` stores the phenotype and cell properties.
...
We can check what phenotypes are present.
...
Tumour_marker = cancer cells Immune_marker1,Immune_marker2 = immune cell
type 1 Immune_marker1,Immune_marker3 = immune cell type 2
Immune_marker1,Immune_marker2,Immune_marker4 = immune cell type 3 OTHER
= other cell types

### Nomenclature

In SPIAT We define as **markers** proteins whose levels where queried
by OPAL, CODEX or another platforms.
...
required for phenotyping, using HALO or inForm or a machine-learning best method 
is recommended.
...
```{r, fig.height=1.6, dpi = 72, out.width="100%"}
...
`r summary_distances[4,"Target"]` is `r summary_distances[4,"Mean"]`'.
...
`r min_summary_dist[4,"Target"]` is `r min_summary_dist[4,"Mean"]`'.
...
[@keren2018structured]. SPIAT generalizes this method for any
...
cell types. The normalized score is normalized for the number of target
...
immune_proportions <- calculate_proportions_of_cells_in_structure(
  formatted_structure, cell_types_of_interest = names_of_immune_cells, 
  feature_colname ="Cell.Type")
...
are assigned to Cluster_NA in the output table. The argument
